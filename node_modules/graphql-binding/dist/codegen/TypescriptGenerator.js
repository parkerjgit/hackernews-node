...
var TypescriptGenerator = /** @class */ (function (_super) {
    ...
    TypescriptGenerator.prototype.getTypeNames = function () {
        ...
        return Object.keys(ast.getTypeMap())
             ...
             */
            var constructorOrder = typeA.constructor.name.localeCompare(typeB.constructor.name);
    ...
    TypescriptGenerator.prototype.renderMainMethodFields = function (operation, fields) {
        ...
        var methods = Object.keys(fields)
            ...
            return "    " + field.name + ": <T = " + _this.renderFieldType(field.type) + ">(args" + (hasArgs ? '' : '?') + ": {" + (hasArgs ? ' ' : '') + field.args
                .map(function (f) { return _this.renderFieldName(f) + ": " + _this.renderFieldType(f.type); })
                .join(', ') + (field.args.length > 0 ? ' ' : '') + "}, info?: GraphQLResolveInfo | string, options?: Options) => " + _this.getPayloadType(operation, isNonNullType(field.type)) + " ";
        })
            .join(',\n');
        return "{\n" + methods + "\n  }";
    };
    TypescriptGenerator.prototype.getPayloadType = function (operation, nonNullType) {
